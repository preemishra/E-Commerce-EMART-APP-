...
exports.viewOrdersServices = async (id) => {
  try {
    let query = `select order_details_id,order_details.order_id,ordered_qty,unit_price,order_status,product_name,descriptions,picture,products.user_id As seller_id,contact,apartment,city,street,pin_code,state,address_type from orders inner join order_details on orders.order_id=order_details.order_id inner join products on order_details.product_id=products.product_id inner join addresses on order_details.address_id=addresses.address_id where orders.user_id=${id} order by addresses.address_id DESC`;
...
exports.viewSellerOrdersServices = async (id) => {
  try {
    let query = `select full_name,orders.order_id,products.product_id,order_details_id,ordered_qty,unit_price,order_status,product_name,contact,apartment,city,street,pin_code,state,address_type from users inner join orders on users.user_id =orders.user_id inner join order_details on orders.order_id=order_details.order_id inner join products on order_details.product_id=products.user_id inner join addresses on order_details.address_id=addresses.address_id where products.user_id=${id}`;
  ...
  try {
    ...
    const result = await db.sequelize.transaction(async (t) => {
      ...
      const order_detailsResp = await db.orderDetail.update(
        orderDetail,
        { where: { order_details_id: bodyData.order_details_id } },
        {
          transaction: t,
        }
      );
    ...
    return {
      success: true,
      data: { order_details_id: bodyData.order_details_id },
...
exports.updateOrderStatusServices = async (bodyData) => {
  try {
    let orderResp;
    orderDetail = {
      updated_by: bodyData.updated_by,
    ...
    const order_detailsResp = await db.orderDetail.update(orderDetail, {
      where: { order_details_id: bodyData.order_details_id },
    });
    orderResp = order_detailsResp;
    return {
